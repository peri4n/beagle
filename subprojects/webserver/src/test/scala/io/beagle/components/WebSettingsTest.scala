...
class WebSettingsTest extends AnyFunSpec with Matchers with EitherValues {
  ...
  describe("The web server") {
    it("can be configured via file") {
      ...
      val expected = WebSettings("subprojects/frontend/dist/", 9000, Postgres("beagle", "fbull", "password", exec = Fixed(3)), SearchSettings(exec = Fixed(1)), SecuritySettings(JwtSettings(30 minutes, "secret")), Fixed(4))
